...
router.get('/dashboard', (req: Request, res: Response) => {
  const incidents = db.institutionalIncidents;
  const totalJourneys = 1240 + db.journeys.length;
...
router.get('/incidents', (req: Request, res: Response) => {
  ...
  let incidents = [...db.institutionalIncidents];
  ...
  if (limit) incidents = incidents.slice(0, parseInt(limit as string));
...
router.get('/analytics', (req: Request, res: Response) => {
  ...
  return res.json({
    ...
    summary: {
      totalIncidents: db.institutionalIncidents.length,
      resolvedRate: Math.round(db.institutionalIncidents.filter(i => i.status === 'resolved').length / db.institutionalIncidents.length * 100),
